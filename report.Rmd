...
mapdata1<-mapdata%>%
  ...
  summarise(postcode=post_code,prop_refer=referrals/sum(referrals)-population/sum(population), lat = cent_lat, lng = cent_long, geometry=geometry)
```
...
leaflet(mapdata1)%>%
  ...
  addPolygons(lat=~lat,lng=~lng, label = ~postcode, color = "gray", fillColor = ~pal(prop_refer))
