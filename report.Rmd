...
map <-
postcode2021 %>%
  filter(postcode_num_2021>2999&postcode_num_2021<4000)%>%
  ggplot() +
...
```
```{r}
mapdata<-referrals_clean3%>%
  group_by(post_code)%>%
  summarise(referrals=n())%>%
  left_join(postcode2021, by=c("post_code"="postcode_2021"))%>%
  left_join(postcode_population, by=c("post_code"="postcode"))%>%
  select(cent_lat, cent_long, population, referrals, post_code,geometry)%>%
  filter(!is.na(population)&!is.na(cent_lat))
...
mapdata1<-mapdata%>%
  ...
  summarise(postcode=post_code,prop_refer=referrals/sum(referrals)-population/sum(population), lat = cent_lat, lng = cent_long, geometry=geometry)%>%mutate(prop_refer=10000*prop_refer)
```
```{r}
map <-
mapdata1 %>%
  ggplot() +
  geom_sf(aes(geometry = geometry, fill=prop_refer),lwd=0, show.legend = TRUE)+  # use the geometry variable
...
```{r}
pal<-colorBin(palette = "Reds", domain=mapdata1$prop_refer,n=5)
```

```{r}
leaflet(mapdata1)%>%
  addTiles()%>%
  addPolygons(lat=~lat,lng=~lng, label = ~postcode, color = "gray", fillColor = ~pal(mapdata1$prop_refer*10^4),fillOpacity = 0.8, weight = 1.0,opacity = 1.0)
