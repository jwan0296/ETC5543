...
mapdata1<-mapdata%>%
  ...
  summarise(postcode=post_code,attractiveness=referrals/sum(referrals)-population/sum(population), lat = cent_lat, lng = cent_long, geometry=geometry)%>%mutate(attractiveness=1000*attractiveness)
```
```{r}
map <-
mapdata1 %>%
  ggplot() +
  geom_sf(aes(geometry = geometry, fill=attractiveness),lwd=0, show.legend = TRUE)+  # use the geometry variable
